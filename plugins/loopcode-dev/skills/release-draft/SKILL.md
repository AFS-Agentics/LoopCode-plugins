---
name: release-draft
description: Draft LoopCode GitHub Release notes from repository evidence. Use when preparing or revising release copy for a version.
---

# Release Draft

Use this skill to produce an English GitHub Release body that feels consistent with recent LoopCode releases while staying grounded in current repo evidence. Evidence may be localized; synthesize the final release copy in English.
...
```bash
gh release view vX.Y.Z --repo AFS-Agentics/LoopCode --json name,tagName,body,url,publishedAt,assets
```
...
```bash
gh api repos/AFS-Agentics/LoopCode/releases/generate-notes -X POST -f tag_name=vX.Y.Z -f target_commitish=<branch-or-sha> -f previous_tag_name=vA.B.C
```
...
Match the reference release's structure unless the user asks for a different format. For current LoopCode releases, use:

```markdown
# LoopCode vX.Y.Z

LoopCode vX.Y.Z is a major release focused on ...
...
**Full Changelog**: https://github.com/AFS-Agentics/LoopCode/compare/vA.B.C...vX.Y.Z
```
