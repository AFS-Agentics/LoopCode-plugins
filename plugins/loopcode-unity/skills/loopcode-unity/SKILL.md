---
name: loopcode-unity
description: Use when a LoopCode thread is connected to Unity Editor through loopcode-unity, when unity_execute_csharp is available, or when the user asks to inspect, automate, capture, or debug a Unity project from the Editor. Provides safe background-first rules for Unity Editor automation and GameView capture.
---
...
Use `LoopCode.Editor` API helpers for high-boilerplate inspection work. `unity_execute_csharp` snippets already import this namespace, so use `Dcu.Type`, `Dcu.Components`, `Dcu.Get`, and `Dcu.Call` directly when they replace substantial reflection or type-search code.
